...
router.all(/^\/aclgroup\/create$/, async(req, res, next) => {
	...
	if(!hasperm(req, 'aclgroup') || (!getperm('developer', ip_check(req) && !hasperm(req, 'aclgroup')))) return res.send(await showError(req, 'permission'));
...
router.post(/^\/aclgroup\/remove$/, async(req, res) => {
	if(!hasperm(req, 'aclgroup') || (!getperm('developer', ip_check(req) && !hasperm(req, 'aclgroup'))) && !hasperm(req, 'admin')) return res.send(await showError(req, 'permission'));
	...
	if(dbdata[0].aclgroup != '차단된 사용자' && !hasperm(req, 'aclgroup') || (!getperm('developer', ip_check(req) && !hasperm(req, 'aclgroup'))))
		return res.send(await showError(req, 'permission'));
...
router.all(/^\/aclgroup$/, async(req, res) => {
	...
	if(req.method == 'POST') do {
		if(group != '차단된 사용자' && !hasperm(req, 'aclgroup') || (!getperm('developer', ip_check(req) && !hasperm(req, 'aclgroup'))))
